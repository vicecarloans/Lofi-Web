        ...
        .then((r) =>
          r.ok
            ...
            : r
                ...
                .then((message) =>
                  Promise.reject(
                    new OIDCMiddlewareError("revokeError", message)
                  )
                )
        )
        .catch(errorHandler)
...
logout.forceLogoutAndRevoke = (context) => {
  ...
  return async (req, res /*, next */) => {
    ...
    const revokeIfExists = (token_hint) =>
      tokens[token_hint]
        ? revokeToken({ token_hint, token: tokens[token_hint] })
        : null;
    ...
    const endOktaSessionEndpoint = `${issuer}/v1/logout?${querystring.stringify(
      params
    )}`;
